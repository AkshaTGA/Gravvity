...
function normalizeRole(value?: string): Member["role"] {
  return value?.toLowerCase().trim() === "coordinator"
    ? "coordinator"
    : "member";
}
...
function normalizeMember(raw: RawMember, index: number): Member | null {
  ...
  return {
    ...
    socials: {
      ...
      linkedin:
        typeof raw.socials?.linkedin === "string" ? raw.socials.linkedin : "",
      twitter:
        typeof raw.socials?.twitter === "string" ? raw.socials.twitter : "",
      instagram:
        typeof raw.socials?.instagram === "string" ? raw.socials.instagram : "",
    },
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        ...
        if (data.length > 0 && !cancelled) setMembers(data);
      }
    } catch {}
