...
export default function AdminDashboard() {
  const formatDate = (input: string | number | Date | undefined) => {
    if (!input) return "";
  ...
  const refreshBlogs = async () => {
    try {
      ...
      setPendingBlogs(blogs.filter((b: any) => b.status !== "approved"));
      setApprovedBlogs(blogs.filter((b: any) => b.status === "approved"));
    } catch (e) {
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-foreground/60 mt-1">
                Manage Gravity members and coordinators
              </p>
            </div>
            ...
            <div className="lg:col-span-2">
                        ...
                        {Array.isArray(p.tags) && p.tags.length > 0 ? (
                          <div className="mt-1 flex gap-1 flex-wrap text-xs text-foreground/70">
                            {p.tags.map((t: string, i: number) => (
                              <span
                                key={i}
                                className="px-2 py-0.5 bg-purple-500/20 rounded"
                              >
                                {t}
                              </span>
                            ))}
              ...
              <div className="mt-12">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <MagicButton onClick={refreshBlogs} heightClass="h-9">
                    Refresh
                  </MagicButton>
                </div>
                {pendingBlogs.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {pendingBlogs.map((b) => (
                      <div
                        key={b.id}
                        className="card-glow p-4 flex items-start gap-4"
                      >
                        <div className="w-16 h-16 rounded-lg overflow-hidden shrink-0 bg-card border border-border flex items-center justify-center">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img
                            src={b.image || "/gravity-logo.png"}
                            alt={b.title || "Blog"}
                            className="w-full h-full object-cover"
                          />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {b.category ? (
                              <span className="px-2 py-0.5 text-xs rounded bg-purple-500/20 text-purple-200 shrink-0">
                                {b.category}
                              </span>
                            ) : null}
                          </div>
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-sm text-purple-300 underline break-all mt-1 inline-block"
                          >
                            {b.mediumUrl}
                          </a>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="px-4 py-2 rounded-lg bg-card border border-border hover:bg-card/80 text-red-400"
                          >
                            Reject
                          </button>
                        </div>
                      ...
                      {approvedBlogs.map((b) => (
                        ...
                        >
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={async () => {
                                if (!confirm("Remove this approved blog?"))
                                  return;
                                try {
                                  const token =
                                    typeof window !== "undefined"
