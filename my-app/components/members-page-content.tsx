...
import { GravityMode } from "@/components/gravity-mode";
...
export function MembersPageContent({
  ...
  const { coordinators, regularMembers, heading, subheading } = useMemo(() => {
    let list = members;
    let pageHeading = "Our Team";
    let pageSubheading = "Meet the talented members of Gravity";
    const isWingFilter = wingFilter && wingFilter.toLowerCase() !== "overall";

    if (isWingFilter) {
      pageHeading = `${wingFilter} Wing`;
      ...
      <GravityMode>
        <main className="min-h-screen m-10 bg-background">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
            ...
            <div
              className={`mt-20 card-glow p-8 text-center ${lightweight ? "" : "slide-in-up"}`}
            >
