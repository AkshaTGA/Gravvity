...
const buildSocialLinks = (socials) => {
  ...
  const socialEntries = [
    ...
    .map(([label, url]) => [label, toSafeUrl(url)])
  ...
  const links = socialEntries
    .map(
      ([label, url]) =>
        `<a class="info-social-link" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">${socialIcons[label] || ''}${escapeHtml(label)}</a>`
    )
    .join('');

  return `<div class="info-socials">${links}</div>`;
};
...
export const displayPaintingInfo = (info) => {
  ...
  const title = escapeHtml(info?.title || 'Member');
  const role = escapeHtml(info?.artist || 'Member');
  const wing = escapeHtml(info?.year || 'Metaverse');
  const description = escapeHtml(info?.description || '');
  const socialLinks = buildSocialLinks(info?.socials);
